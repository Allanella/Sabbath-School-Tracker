...
async function migratePaymentData() {
  ...
  try {
    ...
    // Step 3: Group weekly data by quarter and calculate proper week numbers
    const quarterGroups = {};
    ...
    weeklyData.forEach(record => {
      ...
      quarterGroups[quarterId].push(record);
    });

    // Sort records within each quarter by date
    Object.keys(quarterGroups).forEach(quarterId => {
      quarterGroups[quarterId].sort((a, b) => 
        new Date(a.sabbath_date) - new Date(b.sabbath_date)
      );
    });

    console.log(`📊 Found ${Object.keys(quarterGroups).length} unique quarters\n`);

    // Step 4: Process each weekly data record
    let totalPaymentsCreated = 0;
    let skippedRecords = 0;
    let errorCount = 0;

    for (const [quarterId, records] of Object.entries(quarterGroups)) {
      console.log(`\n📚 Processing Quarter: ${quarterId}`);
      console.log(`   ${records.length} weeks of data`);

      for (let weekIndex = 0; weekIndex < records.length; weekIndex++) {
        const record = records[weekIndex];
        const weekNumber = weekIndex + 1; // Week 1, 2, 3, etc. per quarter
    ...
    console.log(`📚 Unique quarters: ${Object.keys(quarterGroups).length}`);
