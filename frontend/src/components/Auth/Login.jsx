...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Ensure your useAuth login function sends { email, password } 
      // as the backend expects 'email' key
      const response = await login(email, password);
      
      // Use optional chaining (?.) to prevent crashing if the structure changes
      const userData = response?.data?.user || response?.user;
      const role = userData?.role;
      
      if (!role) {
        throw new Error('Authentication succeeded but no role was found.');
      }
      ...
      if (role === 'admin') {
        ...
        // Default role for teachers/members
        navigate('/reports/weekly');
      }
    } catch (err) {
      // Drills down into the specific validation errors we found in the console
      const serverMessage = err.response?.data?.errors?.[0]?.msg;
      const generalMessage = err.response?.data?.message;
      
      setError(serverMessage || generalMessage || 'Login failed. Please check your credentials.');
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-8 relative overflow-hidden">
      ...
      <div className="max-w-md w-full relative z-10">
        <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl p-8 border border-white/30">
          ...
          {/* Error Message */}
          {error && (
            <div className="mb-6 p-4 bg-red-50 border-l-4 border-red-500 rounded-lg flex items-start animate-shake shadow-sm" role="alert">
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            <p className="text-center text-sm text-gray-600">
              Need access? 
              <span className="block mt-1 text-blue-600 font-semibold cursor-pointer hover:underline">
        ...
        <div className="text-center mt-6 space-y-2">
          ...
          <p className="text-xs text-white/90 drop-shadow-md font-medium">
            Developed by <span className="font-bold">Baliddawa Allan</span> • 2026
          </p>
        </div>
      </div>
